...
-(instancetype)initWithConfige:(RealtimeConfige *)confige{
    ...
    if (self = [super init]) {
        ...
        _fftSize = confige.bufferSize;
        [self comminitWithConfige:confige];
    }
    return self;
}

- (void)comminitWithConfige:(RealtimeConfige *)confige {
//    self.frequencyBands = 80;
//    self.startFrequency = 100.0;
//    self.endFrequency = 18000.0;
//    self.spectrumSmooth = 0.5; //缓动系数，数值越大动画越"缓"
    _confige = confige;
    self.frequencyBands = confige.frequencyBands;
    self.startFrequency = confige.startFrequency;
    self.endFrequency = confige.endFrequency;
    self.spectrumSmooth = confige.spectrumSmooth; //缓动系数，数值越大动画越"缓"
...
- (NSArray *)createFrequencyWeights {
    float Δf = self.confige.samplingRate / (float)self.fftSize;
